...
include("backoffice/manage/c_access.php");
    $sqlnews = "select  *  from news where Id ='$newsID'";
...
//Set Display Update time 
		...
		<?if ($inicount%2==0){ $bgcolor="FFFFFF"; $picalign="right";}else{$bgcolor="FFFFFF"; $picalign="left";}?>
								...
								<STRONG><a name="<?=$Id?>"></a><?=$ntittle?></STRONG><BR><?if (isset($nPicture) and $nPicture<>''){?><A HREF="images/newsfile/<?=$nPicture?>" target="imagepop"><IMG 
                                alt="คลิ๊กเพื่อดูขนาดจริง" hspace=5 
                                src="images/newsfile/<?=$nPicture?>" 
	...
	<?$inicount++;}?>
                       <br>
					   <hr  color="#00CCFF"><div align="left"   class="m10bB">ข่าวล่าสุด</div>
...
//Set Display Update time 
  ...
  <tr><td valign="top"><?if (isset($nPicture) and $nPicture<>''){?><A HREF="images/newsfile/<?=$nPicture?>" target="imagepop"><IMG 
                                alt="คลิ๊กเพื่อดูขนาดจริง" hspace=5 
                                src="images/newsfile/<?=$nPicture?>" 
                                height=40 align=<?=$picalign?>  border=0> </A><?}?></td><td align="left" valign="top"><a href="in_current_detail.php?newsID=<?=$Id?>"><?=$ntittle?></a></td></tr>
 <? $inicount++;}?>
				</table>  <br>
