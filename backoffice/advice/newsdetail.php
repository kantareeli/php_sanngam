...
<?
	function getFileExtension($str) {
		...
		if 	(isset($mode) && ($mode=="edit")) {	
				...
				$sqledit = "select  *  from advice  where Id='$Id'";
				//echo  $sqledit;
		...
		if (isset($modesave) && ($modesave=="ADDTEXT")) {	
				if ($albumname=="" || $albumdesc==""){
					...
					$daynow=date("Y-m-d");
					$sql_insert="Insert Into  advice  (ntittle,  ndesc, AdminID, lastUpdate, gaOrder)Values('$albumname', '$albumdesc', '$adminID', '$daynow', '$MAXgaOrder')";
					//echo  $sql_insert;
					$resultinsert = mysql_db_query($database,$sql_insert);
						...
						//start ถ้ามีถาพ
									if(isset($imgfile) AND $imgfile<>''){
												...
												<?}else{
														$uploaddir = "../../images/advicefile/";
														//To creat filename 
															$sqlLAST_ID = "select  max(Id) LAST_ID from advice";
														...
														if (copy($imgfile,"$newfile2")) {
																//Update Database
																$sql_update="update  advice set nPicture='$filesavename' where Id='$LAST_ID'";
																//echo  $sql_insert;
				...
				if ($albumname=="" || $albumdesc==""){
					...
					include("../../initial/connect.php");
					$daynow=date("Y-m-d");
					$sql_update = "update advice set ntittle='$albumname',  ndesc='$albumdesc', lastUpdate='$daynow', AdminID='$adminID' where Id='$Id'";
					//echo  $sql_update;
						...
						//start ถ้ามีถาพ
									if(isset($imgfile) AND $imgfile<>''){
												...
												<?}else{
														...
														if (copy($imgfile,"$newfile2")) {
																//Update Database
																$sql_update="update  advice set nPicture='$filesavename' where Id='$Id'";
																//echo  $sql_insert;
																$result = mysql_db_query($database,$sql_update);
																if ($result){?>
																		<script language = "javascript">
...
<img onMouseOver="bbc_highlight(this, true);" 	onMouseOut="if (window.bbc_highlight) bbc_highlight(this, false);" 	src="../images/bbc/bold.gif" 	align="bottom" width="23" height="22" alt="ตัวหนา" title="ตัวหนา" 	style="background-image: url(../images/bbc/bbc_bg.gif); 
	...
	<TR>
		<TD>ภาพประกอบ :</TD>
		<TD><IMG name="upload_img" SRC="../../images/advicefile/<?=$nPicture?>" WIDTH="480"  BORDER="1" ALT="Image Sample" > ความกว้าง 480 pixel<br><input type="file" name="imgfile" onchange = "getimg()" style="cursor:hand" size="50"><input type="hidden" name="MAX_FILE_SIZE" value=""></TD>
	</TR>
	<TR>
