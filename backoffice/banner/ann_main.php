...
include("../../initial/connect.php");
	$sqlcount = "select  count(*) CC  from cbanner ";
...
if (isset($mode) && ($mode=="MOVEUP")) {	
	$sqlA = "select  gaOrder  from cbanner  where   Id ='$f_id'";
	//echo $sqlA."<BR>";
	...
	$sqlB = "select  Id  from cbanner  where   gaOrder ='$FAQ_ORDER'";	
	//echo  $sqlB."<BR>";
	...
	$sql1 = "update  cbanner set gaOrder=gaOrder-1  where   Id ='$f_id'"; 
	//echo  $sql1."<BR>";
	$updatestmt1 = mysql_query($sql1 ) or die ("Invalid query sql1 : " . mysql_error());

	$sql2 = "update  cbanner set gaOrder=gaOrder+1 where   Id ='$Id_upper'"; 
	//echo  $sql2."<BR>";
...
<!--Out Table----------------------------------------------------------------------------------------------------------------------------------------------->
<table cellpadding="0" cellspacing="0" border="0" width="800" height="">
	<tr>
		<td valign="top">

			<table cellpadding="0" cellspacing="0" border="0" width="100%"  bgcolor="#F6FBFE">
				<tr>
					<td valign="top" bgcolor="">
<!--Middle Table-------------------------------------------------------------------------------------------------------------------------------------------------->	
						<table cellpadding="0" cellspacing="0" border="0" width="100%" height="">
							<tr>
								<td align="center" valign="top" colspan="1" width="85%">
                  ...
                  src="../images/menu_total2.gif" width=170 style="cursor:hand"></a></td>
														...
														<td align="center" width="60"><FONT FACE="ms sans serif" SIZE="1" COLOR="#FFFFFF" ><B>จัดลำดับ</B></FONT></td>
<!-- 													<td align="center"><FONT FACE="ms sans serif" SIZE="1" COLOR="#FFFFFF"><B>ภาพประกอบ</B></FONT></td> -->
															<td align="center" width="400"><FONT FACE="ms sans serif" SIZE="1" COLOR="#FFFFFF"><B>หัวข้อ</B></FONT></td>
...
$Id= mysql_result($stmt,$inicount,"Id");
$ntittle= mysql_result($stmt,$inicount,"ntittle");
$nPicture= mysql_result($stmt,$inicount,"nPicture");
...
?>
			...
			<td align="center">
								<?if ($gaOrder<>1){?>&nbsp;   
											<IMG SRC="../images/upicon.gif"  BORDER="0" ALT="เลื่อนขึ้น"  onclick="moveUp(<?print $Id?>)" style="cursor:hand">&nbsp;    
								<?}?>
								<?if ($gaOrder<>$MAXgaOrder){?>&nbsp;   
											<IMG SRC="../images/downicon.gif"  BORDER="0" ALT="เลื่อนลง"onclick="moveDown(<?print $Id?>)" style="cursor:hand">&nbsp;   
								<?}?></td>
<!-- 			<td align="center"><FONT FACE="ms sans serif" SIZE="1" COLOR="#000000">
			<?if ($nPicture<>0){?><img src="../../images/galleryfile/<?=$picHilight?>" width="80" height="60" align="center" >
			<?}else{?>NO IMAGE HIGHLIGHT<?}?></FONT></td> -->

			<td align="left">&nbsp;<FONT FACE="ms sans serif" SIZE="1" COLOR="#000000"><B><IMG name="upload_img" SRC="../../images/beautyalertfile/<?=$nPicture?>"   BORDER="0" ALT="Image Sample" > <?print $ntittle?>...</B></FONT>&nbsp;</td>
															
			<td align="center" width="17%">&nbsp;<FONT FACE="ms sans serif" SIZE="1" COLOR="#000000"><?print$update_date?></FONT>&nbsp;</td>
			
			<td align="center">
					<A style="CURSOR: hand"     href="newsdetail.php?mode=edit&Id=<?=$Id?>"><IMG     alt=แก้ไขข้อมูล   src="../images/icon_edit.gif"   border=0></A>
			</td>
				
			<td align="center"><?if ($gaOrder==$MAXgaOrder){?><A style="CURSOR: hand"  onclick="goto_del(<?=$Id?>)"   ><IMG     alt=ลบข้อมูล   src="../images/icon_delete.gif"   border=0></A><?}?></td>
...
<!------------ TOOL ----------------------------->
														<tr valign="center" >
															<td colspan="8" bgcolor="#4FAEFF" align=center>
																			...
																			<a href="newsdetail.php?mode=add&Id=<?=$Id?>"
...
function goto_del(code){
	if  (!confirm('คุณต้องการจะลบข้อมูลนี้ใช่หรือไม่')) {
		...
		frmmain.action = 'ann_del.php?Id='+code;
		frmmain.submit();
	}
}
